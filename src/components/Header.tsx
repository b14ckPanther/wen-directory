...
import { Search, Menu, UserCircle, PlusCircle, X, Home, LayoutGrid, MapPin } from 'lucide-react';
...
export default function Header() {
  ...
  // This effect now correctly handles both modals to prevent background scrolling
  useEffect(() => {
      ...
      <header className="sticky top-0 z-50 bg-navy/70 backdrop-blur-xl border-b border-gold/20">
        <nav className="container mx-auto flex items-center justify-between p-4 h-20 text-gray">
          ...
          <div className="flex items-center gap-2 md:hidden">
            <button onClick={() => setIsLocationOpen(true)} className="text-gray hover:text-gold transition-colors p-2">
              <MapPin size={28} />
            </button>
            <button onClick={() => setIsMenuOpen(true)} className="text-gray hover:text-gold transition-colors p-2">
              <Menu size={28} />
            </button>
          </div>
        </nav>
        ...
        <div className="container mx-auto p-4 flex flex-col h-full">
          ...
          <div className="flex flex-col items-center justify-center flex-grow gap-6 text-xl font-semibold">
             <div className="relative w-full max-w-sm mb-4">
              <input type="text" placeholder="ابحث عن أي خدمة..." className="w-full bg-[#1B2A41] text-gray border border-gold/50 rounded-full py-3 pr-12 pl-5 focus:outline-none focus:ring-2 focus:ring-gold/50" />
              <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-gold"><Search size={22} /></div>
            </div>

            {navLinks.map((link) => {
              const Icon = link.icon;
              return (
                <a key={link.text} href={link.href} className="w-full text-center p-4 rounded-lg flex items-center justify-center gap-3 text-gray hover:text-navy hover:bg-gold transition-all">
                  <Icon size={24} />{link.text}
                </a>
            )})}
            <a href="#" className="w-full text-center p-4 rounded-lg flex items-center justify-center gap-3 text-gray hover:text-navy hover:bg-gold transition-all">
              <PlusCircle size={24} />انضم لنا
            </a>
          </div>

          <div className="py-6">
             <a href="#" className="w-full p-4 rounded-lg flex items-center justify-center gap-3 text-lg font-semibold text-gold border-2 border-gold hover:bg-gold hover:text-navy transition-all">
              <UserCircle size={24} />تسجيل الدخول
            </a>
          </div>
        </div>
      </div>
      
      {/* This renders the Location Selector Modal when it's open */}
      <LocationSelector
